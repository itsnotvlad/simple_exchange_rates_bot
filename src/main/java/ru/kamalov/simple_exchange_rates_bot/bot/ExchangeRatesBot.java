package ru.kamalov.simple_exchange_rates_bot.bot;
...
import ru.kamalov.simple_exchange_rates_bot.service.ExchangeRatesService;
...
public class ExchangeRatesBot extends TelegramLongPollingBot {
    ...
    public String getBotUsername() {
        return "simple_exchange_rates_bot";
    }
    ...
    private void gbrCommand(Long chatId) {
        String formattedText;
        try {
            var gbr = exchangeRatesService.getGBRExchangeRate();
            var text = "Курс фунта стерлингов на %s составлякт %s рублей";
            formattedText = String.format(text, LocalDate.now(), gbr);
        } catch (ServiceException e) {
            LOG.error("Ошибка получения курса фунта стерлингов", e);
            formattedText = "Не удалось получить текущий курса фунта стерлингов. Попробуйте позже.";
        }
        sendMessage(chatId, formattedText);
    }
